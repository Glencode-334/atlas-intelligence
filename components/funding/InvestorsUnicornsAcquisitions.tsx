...
export function InvestorsUnicornsAcquisitions() {
    ...
    <ul className="space-y-3">
      {investors.map((inv, i) => (
        ...
        >
          ...
          {/* Logo */}
          <div className="ml-1 flex h-7 w-7 items-center justify-center overflow-hidden rounded-md">
            {inv.mark}
          </div>
  ...
  <div className="grid grid-cols-5 gap-3">
    {unicorns.map((u) => (
      <div
        key={u.name}
        className="flex flex-col items-center rounded-lg border border-[#F3F3F1] bg-[#FCFCFC] px-2.5 py-3"
      >
        <div
          className="mb-2 flex h-10 w-10 items-center justify-center rounded-lg border border-[#ECEAE5]"
          style={{ background: u.bg }}
        >
          {u.mark}
        </div>

        <h4 className="text-[9px] font-semibold text-[#111] text-center">
          {u.name}
        </h4>

        <div className="mt-2 text-[14px] font-bold leading-none text-[#FF5A4E]">
          {u.val}
        </div>

        <span className="mt-1 text-[9px] font-medium text-[#FF8B82]">
          Valuation
        </span>

        <span className="mt-2 text-[9px] text-[#8E8E93]">
          {u.date}
        </span>
      </div>
    ))}
  ...
  <div className="mb-4 flex items-center justify-between">
    ...
    <a
      href="#"
      className="text-[11px] font-semibold text-[#FF5A4E]"
    >
  ...
  <ul className="space-y-2.5">
    {acq.map((a) => (
      ...
      >
        <span
          className="flex h-6 w-6 items-center justify-center overflow-hidden rounded-md border border-[#EFEDE8]"
        ...
        <span className="flex-1 text-[11px]">
          <span className="font-semibold">{a.buyer}</span>{" "}
          <span className="text-[#6B6B73]">acquired</span>{" "}
